...
def main():
    ...
    if args.split_validation:
        ...
        test_annotations    = 'datas/study_file.txt'
    ...
    for epoch in range(runner.get('epoch'),runner.get('max_epochs')):
        ...
        train(model, runner, lr_update_func, device, epoch, data_cfg.get('train').get('epoches'), data_cfg.get('study_file'), meta)
        validation(model, runner, data_cfg.get('study_file'), device, epoch, data_cfg.get('train').get('epoches'), meta)
